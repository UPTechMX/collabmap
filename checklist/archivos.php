...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$('#regresarArch').click(function(event) {
			$('#pregunta').load(rz+'checklist/ultimaPreg.php',{
		...
		$('#multimedia').on('click', '.multDel', function(event) {
			...
			conf('¿Seguro que deseas eliminar la fotografía?',{multId:multId,ele:$(this)},function(e){
				...
				var rj = jsonF('checklist/json/json.php',{acc:4,vId:<?php echo $_POST['vId'];?>,mId:e.multId});
				console.log(rj);
				var r = $.parseJSON(rj);
				if(r.ok == 1){
		...
		$('#permisoImagen').change(function(event) {
			...
			// console.log(dat);
			var rj = jsonF('checklist/json/json.php',{dat:dat,acc:7,vId:vId});
