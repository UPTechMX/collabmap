<?php  
	...
	if( !is_numeric($_POST['consultationsId']) || !is_numeric($_POST['chkId']) ){
		exit('ERROR');
	}
	// print2($_POST);
	...
	// $JOINS = getLJTrgt($_POST['nivelMax'],$_POST['padre'],$_POST['consultationsId'],'structure');
	// print2($JOINS);

	$info = $db->query("SELECT t.name as tName, p.name as pName
		FROM Targets t
		LEFT JOIN Projects p ON p.id = t.projectsId
		WHERE t.id = $_POST[consultationsId]")->fetchAll(PDO::FETCH_ASSOC)[0];
	...
	$params['camposExt'] = "*";
	$params['where'] = " AND v.finalizada = 1 AND type = 'cons' ";
	...
	$dims = $db->query("SELECT * FROM Dimensiones 
		WHERE elemId = $_POST[consultationsId] AND type='structure' ")->fetchAll(PDO::FETCH_ASSOC);
	...
	foreach ($dims as $d) {
		$csv .= '<ss:Cell><ss:Data ss:Type="String">'.$d['nombre'].'</ss:Data></ss:Cell>'."\n";	
	}
