<?php
	...
	$consultationChecklist = $db->query("SELECT * FROM ConsultationsChecklist WHERE id = $_REQUEST[consChk]")->fetchAll(PDO::FETCH_ASSOC)[0];
	if(empty($consultationChecklist)){
		exit('EXIT');
	}



	$checklistId = $consultationChecklist['checklistId'];

	// print2($consultationChecklist);
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$('.preguntaAnalysis').click(function(event) {
			...
			var consChk = <?php echo $_REQUEST['consChk']; ?>;
			if($(datoPreg).is(":visible")){
				if(typeof pregsDesp[pId] == 'undefined'){
					// console.log('carga');
					$(datoPreg).load(rz+'analysis/consultations/checklist/pregunta.php',{
						consChk:consChk,
						pId:pId,
		...
		$('#dwlAns').click(function(event) {
			...
			var consultationsId = <?php echo $consultationChecklist['consultationsId']; ?>;

			$('<form>')
			.attr({
				id: 'descPrueba',
				action: rz+'analysis/consultations/checklist/json/getDatosXLS.php',
				target:'_blank',
				method:'post'
			})
			.html(
