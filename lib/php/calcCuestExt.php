...
function getChkId($visita){
	...
	$sql = "SELECT v.*, t.tipo as tChk, r.repeticionesId, t.id as tId, t.marcasId as mId
		FROM Visitas v
		LEFT JOIN Rotaciones r ON v.rotacionesId = r.id
		LEFT JOIN Tiendas t ON t.id = r.tiendasId
		LEFT JOIN Marcas m ON r.tiendasId = m.id
		WHERE v.id = $visita ";
	// echo "$sql<br/>";
	$datVis = $db->query($sql)->fetch(PDO::FETCH_ASSOC);

	// print2($datVis);

	$sql = "SELECT DISTINCT(id) FROM ChecklistExt 
		WHERE repeticionesId = $datVis[repeticionesId] 
...
function estructura($chkId){
	...
	// echo $chkId;
	$tipoProm = $db->query("SELECT tipoProm, tipoAnalisis FROM ChecklistExt WHERE id = $chkId")->fetchAll(PDO::FETCH_NUM)[0];
	$chk['tipoProm'] = $tipoProm[0] != ''? $tipoProm[0] : 1;
	$chk['tipoAnalisis'] = $tipoProm[1] != ''? $tipoProm[1] : 1;

	$chk['conds'] = $db->query("SELECT * 
		FROM CondicionalesExt WHERE aplicacion = 'chk' AND eleId = $chkId ORDER BY orden")->fetchAll(PDO::FETCH_ASSOC);
	...
	foreach ($bloques as $b) {
		...
		$chk['bloques'][$b['identificador']]['conds'] = $db->query("SELECT * 
			FROM CondicionalesExt WHERE aplicacion = 'bloque' AND eleId = $b[id] ORDER BY orden")->fetchAll(PDO::FETCH_ASSOC);
		...
		foreach ($areas as $a) {
			...
			$chk['bloques'][$b['identificador']]['areas'][$a['identificador']]['conds'] = $db->query("SELECT * 
				FROM CondicionalesExt WHERE aplicacion = 'area' AND eleId = $a[id] ORDER BY orden")->fetchAll(PDO::FETCH_ASSOC);
			...
			$pregs = $db->query("SELECT p.id,p.identificador,p.influyeValor,p.subareasId,p.orden,p.puntos, p.pregunta,
				t.siglas as tsiglas, t.nombre as nTipo FROM PreguntasExt p
			...
			foreach ($pregs as $p) {
				...
				[$p['identificador']]['conds'] = $db->query("SELECT * 
					FROM CondicionalesExt WHERE aplicacion = 'preg' AND eleId = $p[id] ORDER BY orden")->fetchAll(PDO::FETCH_ASSOC);
				...
				if($p['tsiglas'] == 'sub'){
					...
					foreach ($subpregs as $sp) {
						...
						['preguntas'][$p['identificador']]['subpregs'][$sp['identificador']]['conds'] = $db->query("SELECT * 
							FROM CondicionalesExt WHERE aplicacion = 'preg' AND eleId = $sp[id] ORDER BY orden")->fetchAll(PDO::FETCH_ASSOC);
...
function resultadosDB($visita,$chk){
	global $db;
	// print2($chk);
	...
	$sql = "SELECT p.identificador as pId,p.identificador, rv.*, p.orden, r.id as rId,
		...
		FROM RespuestasUsrExt rv
		LEFT JOIN PreguntasExt p ON rv.preguntasId = p.id
		LEFT JOIN Tipos t ON p.tiposId = t.id
		LEFT JOIN RespuestasExt r ON rv.respuesta = r.id 
		WHERE visitasExtId = $visita " ;
...
function resultados($res,$chk){
	global $db;
	// print2($chk);
	$arr = array();
	foreach ($res as $r) {
		$arr[$r['identificador']] = array();

		$tmp = $r;
		$tmp['pId'] = $r['identificador'];
		$tmp['orden'] = $r['orden'];
		$tmp['rId'] = $r['respuesta'];
		$tmp['valResp'] = $r['valResp'];
		$tmp['rNom'] = $r['nomResp'];
		$arr[$r['identificador']][] = $tmp;
	}
...
function respVisitas($elem,$reps,$proyectoId){
	...
	if($nivel == $numDim){
		...
		$sql = "SELECT v.id as vId, p.identificador as pId,p.identificador, rv.*, 
			...
			LEFT JOIN RespuestasExt resp ON rv.respuesta = resp.id
...
function insertaCacheVisita($visitasId,$c){
	global $db;
	$del = $db->prepare("DELETE FROM CalculosVisitaExt WHERE visitasId = :visitasId");
