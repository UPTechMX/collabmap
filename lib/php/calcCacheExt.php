...
function getBloques($proyectoId,$elem){
	...
	LEFT JOIN Repeticiones rep ON chk.repeticionesId = rep.id
	WHERE rep.proyectosId = $proyectoId AND (b.elim IS NULL OR b.elim != 1) AND (chk.elim IS NULL OR chk.elim != 1)
	GROUP BY clase
	ORDER BY b.orden";
...
function promTotComp($elem,$reps,$proyectoId,$params){
	global $db;
