...
class Checklist {
	...
	public function getGeneral(){
		...
		if(empty($this->general)){
			$general = $db->query("SELECT * FROM ChecklistExt WHERE id = $this->id")->fetchAll(PDO::FETCH_ASSOC)[0];
	...
	public function getVisita($vId){
		global $db;
		if(empty($this->visita[$chkId])){
			$visita = $db->query("SELECT * FROM Visitas WHERE id = $vId")->fetchAll(PDO::FETCH_ASSOC)[0];
			$this->visita = $visita;
			return $visita;
		}else{
			return $this->visita;
...
function resultadosEXT($visita,$chk){
	...
	// $sql = "SELECT p.identificador as pId,p.identificador, p.orden, 
	// 	CASE 
	// 		WHEN t.siglas = 'mult' THEN '-'
	// 		ELSE ''
	// 	END as valResp,
			...
			foreach ($a['preguntas'] as $pId => $p) {
				...
				switch ($p['tipo']) {
					case 'sub':
						...
						break;
					case 'mult':
						$chk['bloques'][$bId]['areas'][$aId]['preguntas'][$pId]['respuesta'] = '-';
