  ...
  Selectpicker.prototype = {
    ...
    keydown: function (e) {
      ...
      if (isArrowKey) { // if up or down
        ...
        index =  true ? $items.index($items.filter('.active')) : that.activeIndex;
        // console.log(isVirtual,$items.index($items.filter('.active')),that.activeIndex)
        if (index === undefined) index = -1;

        if (index !== -1) {
          // console.log(that.selectpicker.current.elements,index,position0);
