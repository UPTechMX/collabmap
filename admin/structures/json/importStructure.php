<?php
	...
	$insDims = $db->prepare("INSERT INTO Dimensiones SET elemId = ?, nombre = ?,nivel = ?, type=?");
	...
	if (($handle = fopen(raiz()."externalFiles/$_POST[file]", "r")) !== FALSE) {
		while (($col = fgetcsv($handle, 0, ",")) !== FALSE) {
			...
			if($row == 0){
				...
				$buscaDimsP = $db->prepare("SELECT id FROM Dimensiones 
					WHERE elemId = $elemId AND type = ?
					ORDER BY nivel");
				$buscaDimsP -> execute(array($_POST['type']));

				$buscaDims = $buscaDimsP -> fetchAll(PDO::FETCH_NUM);
				...
				if(count($buscaDims) > 0){
					...
					foreach ($dims as $k => $dim) {
						$insDims -> execute(array($elemId,$dim,$k+1,$_POST['type']));
