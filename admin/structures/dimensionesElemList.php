<?php  
	...
	$dimElem = $db->query("SELECT de.* 
		...
		WHERE elemId = $_POST[elemId] AND type = '$_POST[type]' 
			AND de.padre = $_POST[padreId] ORDER BY nombre")->fetchAll(PDO::FETCH_ASSOC);
	$dims = $db->query("SELECT id FROM Dimensiones 
		WHERE elemId = $_POST[elemId] AND type = '$_POST[type]' ORDER BY id")->fetchAll(PDO::FETCH_NUM);
	// print2($dims);
	$numDims = count($dims);
	for($i = 0;$i<$numDims;$i++){
	...
	$(document).ready(function() {
		...
		$('#dimensionesElemList_<?php echo $_POST['dimensionId'];?> .delElemDim').click(function(event) {
			...
			conf('¿Estás seguro que deseas borrar el elemento?<br/> '+
				'Se perderá toda la información relacionada.',{elemId:elemId,elem:$(this)},function(e){
					var rj = jsonF('admin/structures/json/json.php',{acc:6,elemId:e.elemId});
					// console.log(rj);
					var r = $.parseJSON(rj);
					if(r.ok == 1){
		...
		<?php if (!empty($dimSig)): ?>
			$('#dimensionesElemList_<?php echo $_POST['dimensionId'];?> .verDimElem').click(function(event) {
				...
				.load(rz+'admin/structures/dimensionesElems.php',{
					ajax:1,
					elemId:<?php echo $_POST['elemId']; ?>,
					type:"<?php echo $_POST['type']; ?>",
