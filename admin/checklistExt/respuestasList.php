<?php  
	...
	checaAcceso(50);
	$resps = $db->query("SELECT * FROM RespuestasExt 
	...
	$(document).ready(function() {
		...
		$( "#respuestasList_<?php echo $_POST['pregId']; ?> .delResp" ).click(function(event) {
			...
			conf('¿Está seguro que desea elilminar la respuesta?',{respId:respId,pregId:pregId},function(e){
				var rj = jsonF('admin/checklistExt/json/json.php',{datos:{id:respId,elim:1},acc:2,opt:8});
				var r = $.parseJSON(rj);
				if(r.ok == 1){
					$('#respuestasList_'+pregId).load(rz+'admin/checklistExt/respuestasList.php',
		...
		$( "#respuestasList_<?php echo $_POST['pregId']; ?> .edtResp" ).click(function(event) {
			var respId = this.id.split('_')[1];
			var pregId = <?php echo $_POST['pregId']; ?>;
			popUp('admin/checklistExt/respuestasAdd.php',{respId:respId,pregId:pregId},function(e){},{});
		...
		// 	var respId = this.id.split('_')[1];
		// 	popUp('admin/checklistExt/condiciones.php',{eleId:respId,aplicacion:'resp'},function(e){},{});
		// });

		$('#respuestasList_'+<?php echo $_POST['pregId']; ?>+' #addSiNo').click(function(event) {
			var pregId = $(this).closest('.respuestas').attr('id').split('_')[1];
			// console.log(pregId);
			var rj = jsonF('admin/checklistExt/json/addSiNo.php',{pregId:pregId,opt:1});
			console.log(rj);
			var r = $.parseJSON(rj);
			if(r.ok == 1){
				$('#respuestasList_'+pregId).load(rz+'admin/checklistExt/respuestasList.php',
			...
			// popUp('admin/checklistExt/respuestasAdd.php',{pregId:pregId},function(e){},{});
		...
		$('#respuestasList_'+<?php echo $_POST['pregId']; ?>+' #add1Al10').click(function(event) {
			var pregId = $(this).closest('.respuestas').attr('id').split('_')[1];
			// console.log(pregId);
			var rj = jsonF('admin/checklistExt/json/addSiNo.php',{pregId:pregId,opt:2});
			console.log(rj);
			var r = $.parseJSON(rj);
			if(r.ok == 1){
				$('#respuestasList_'+pregId).load(rz+'admin/checklistExt/respuestasList.php',
