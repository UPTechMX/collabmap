<?php  
	...
	$conds = $db->query("SELECT * FROM CondicionalesExt 
	...
	$(document).ready(function() {
		$('.delCond').click(function(event) {
			...
			conf('¿Está seguro que desea elilminar la condición?',{condId:condId},function(e){
				var rj = jsonF('admin/checklistExt/json/json.php',{acc:4,condId:e.condId});
				var r = $.parseJSON(rj);
				if(r.ok == 1){
		...
		$('#saveCondsOrden').click(function(event) {
			...
			// console.log(bloques);
			var rj = jsonF('admin/checklistExt/json/json.php',{bloques:bloques,acc:5,opt:9});
			var r = $.parseJSON(rj);
			if(r.ok == 1){
				$('#saveCondsOrden').hide();
			}
