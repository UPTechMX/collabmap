...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$('#bloquesSort .edtBloque').click(function(event) {
			...
			popUp('admin/checklistExt/bloquesAdd.php',{bloqueId:bloqueId,checklistId:checklistId},function(e){},{});
		});

		$('#bloquesSort .verBloque').click(function(event) {
			var bloqueId = this.id.split('_')[1];
			var nomBloq = $('#nomBloq_'+bloqueId).text();
			$('#areas').load(rz+'admin/checklistExt/areas.php',{bloqueId:bloqueId,nomBloq:nomBloq});
			$('#preguntas').empty();
		});
		...
		$('#bloquesSort .delBloque').click(function(event) {
			...
			conf('¿Está seguro que desea elilminar el bloque del checklist?',{bloqueId:bloqueId},function(e){
				var rj = jsonF('admin/checklistExt/json/json.php',{datos:{id:e.bloqueId,elim:1},acc:2,opt:5});
				var r = $.parseJSON(rj);
				if(r.ok == 1){
					$('#bloquesList').load(rz+'admin/checklistExt/bloquesList.php',
		...
		$('#bloquesSort .condBloque').click(function(event) {
			...
			popUp('admin/checklistExt/condiciones.php',{eleId:areaId,aplicacion:'bloque'},function(e){},{});
		});
