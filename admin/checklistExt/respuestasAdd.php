<?php  
	...
	checaAcceso(50);
	if($_POST['respId'] != ''){
		$datP = $db-> query("SELECT * FROM RespuestasExt WHERE id = $_POST[respId]")->fetch(PDO::FETCH_ASSOC);
	}

	// print2($datP);
	$sql = "SELECT p.id as pId, a.id as aId, b.id as bId, c.id as cId
		FROM PreguntasExt p 
		LEFT JOIN AreasExt a ON p.areasId = a.id
		LEFT JOIN BloquesExt b ON a.bloquesId = b.id
		LEFT JOIN ChecklistExt c ON b.checklistId = c.id
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$('#env').click(function(event) {
			...
			// console.log(dat);
			if(allOk){
				var rj = jsonF('admin/checklistExt/json/json.php',{datos:dat,acc:acc,opt:8});
				// console.log(rj);
				var r = $.parseJSON(rj);
				// console.log(r);
				if(r.ok == 1){
					$('#popUp').modal('toggle');
					$('#respuestasList_<?php echo $_POST["pregId"]; ?>').load(rz+'admin/checklistExt/respuestasList.php',
