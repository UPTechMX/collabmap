<?php  
	...
	$resumen = $db->query("SELECT resumen FROM ChecklistExt WHERE id = $chkId")->fetch(PDO::FETCH_NUM)[0];
	$chkInf = $db->query("SELECT * FROM ChecklistExt WHERE id = $chkId")->fetchAll(PDO::FETCH_ASSOC);
	// print2($chkInf);
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		subArch($('#addImg'),2,'Ext_<?php echo $_POST['checklistId'];?>_IMG_','jpg,png,svg',false,function(a){
			...
			var rj = jsonF('admin/checklistExt/json/json.php',{datos:dat,acc:1,opt:10});
			console.log(rj);
			var r = $.parseJSON(rj);
			if(r.ok == 1){
				$('<tr>')
		...
		$('#tablaImg').on('click', '.elimImg', function(event) {
			...
			conf('¿Está seguro que desea elilminar la imágen del checklist?',{imgId:imgId},function(e){
				var rj = jsonF('admin/checklistExt/json/json.php',{datos:{id:e.imgId},acc:7,opt:10});
				// console.log(rj);
				var r = $.parseJSON(rj);
				if(r.ok == 1){
					$('#img_'+e.imgId).remove();
		...
		$('#tablaImg').on('click', '.verImg', function(event) {
			event.preventDefault();
			var imgId = $(this).closest('tr').attr('id').split('_')[1];
			popUp('admin/checklistExt/imgVer.php',{imgId:imgId},function(e){},{});
		...
		$('#gResumen').click(function(event) {
			...
			// console.log(dat);
			var rj = jsonF('admin/checklistExt/json/json.php',{datos:dat,acc:2,opt:4});
			// console.log(rj);
		});
...
<div class="col-md-6">
	...
	<table id="tablaImg" class="table">
		<?php 
			$imgs = $db->query("SELECT * FROM ChecklistImagenesExt WHERE checklistId = $chkId")->fetchAll(PDO::FETCH_ASSOC);
			foreach ($imgs as $img) {
		?>
