<?php  
	...
	$usuarios = $db->query("SELECT id as val, CONCAT(IFNULL(aPat,''),' ',IFNULL(aMat,''), ', ', IFNULL(nombre,'')) as nom, 'clase' as clase 
		FROM usrAdmin WHERE id > 1 ORDER BY aPat, aMat, nombre")->fetchAll(PDO::FETCH_ASSOC);
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$('#env').click(function(event) {
			...
			if(allOk){
				var rj = jsonF('admin/administracion/reconocimientos/json/json.php',{datos:dat,acc:acc,opt:1});
				// console.log(rj);
				var r = $.parseJSON(rj);
				// console.log(r);
				if(r.ok == 1){
					$('#popUp').modal('toggle');
					$('#reconocimientosList').load(rz+'admin/administracion/reconocimientos/reconocimientosList.php',{ajax:1});
				}
			}
