	...
	$(document).ready(function() {
		...
		$('#env').click(function(event) {
			...
			if(allOk){
				...
				var rj = jsonF('admin/administracion/equipos/json/json.php',{datos:dat,acc:acc,opt:4});
				console.log(rj);
				var r = $.parseJSON(rj);
				// console.log(r);
				if(r.ok == 1){
					$('#popUp').modal('toggle');
					$('#dimensionesList').load(rz+'admin/administracion/equipos/dimensionesList.php',{ajax:1,areasId:<?php echo $_POST['areasId']; ?>});
					<?php if(isset($_POST['dimensionId'])): ?>
						$("#dimCol_<?php echo $_POST['dimensionId'];?>").text(dat.nombre);
					<?php else:?>
						$('#dimensionesArbol').load(rz+'admin/administracion/equipos/dimensionesArbol.php',{ajax:1,areasId:<?php echo $_POST['areasId']; ?>});
					<?php endif;?>
				}
			}
