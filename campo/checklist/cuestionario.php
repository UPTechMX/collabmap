	...
	$(function() {
		...
		$('.areaBtn').click(function(event) {
			...
			if( !$(this).hasClass('disabled') ){			
				...
				$('#pregunta').empty();
				if(aId == 'area_general'){
					$('#pregunta').load(rz+'campo/checklist/areaGeneral.php',{
						chkId:chkId,
						vId:vId
					},function(){});
				}else if(aId == 'area_archivos'){
					// console.log('aa');
					$('#pregunta').load(rz+'campo/checklist/archivos.php',{
						vId:vId
					} ,function(){});
				}else if(aId == 'area_instSug'){
					// console.log('aa');
					$('#pregunta').load(rz+'campo/checklist/instSug.php',{
						vId:vId
					} ,function(){});
				}else if(aId == 'area_inst'){
					// console.log('aa');
					$('#pregunta').load(rz+'campo/checklist/inst.php',{
						vId:vId
					} ,function(){});
				}else if(aId == 'area_vistaGral'){
					// $('#pregunta').empty();
					$('#pregunta').load(rz+'campo/checklist/divVisita.php',{
						vId:vId,
						div:1
					} ,function(){});
				}else{
	...
	$(function() {
		...
		$('#pregunta').on('click', '#siguienteGral', function(event) {
			...
			if(allOk){
				...
				var rj = jsonF('campo/checklist/json/json.php',{acc:2,datos:datos,vId:<?php echo $_POST['vId'];?>});
				// console.log(rj);
				var r = $.parseJSON(rj);
